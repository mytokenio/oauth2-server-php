...
class JwtAccessToken extends AccessToken
{
     ...
     *
     * @param mixed  $app_id           - Client identifier related to the access token.
     ...
     */
    public function createAccessToken($app_id, $user_id, $scope = null, $includeRefreshToken = true)
    {
        // payload to encrypt
        $payload = $this->createPayload($app_id, $user_id, $scope);

        /*
         * Encode the payload data into a single JWT access_token string
         */
        $access_token = $this->encodeToken($payload, $app_id);
        ...
        $this->tokenStorage->setAccessToken($token_to_store, $app_id, $user_id, $this->config['access_lifetime'] ? time() + $this->config['access_lifetime'] : null, $scope);
        ...
        if ($includeRefreshToken && $this->refreshStorage) {
            ...
            $this->refreshStorage->setRefreshToken($refresh_token, $app_id, $user_id, $expires, $scope);
     ...
     * @return mixed
     */
    protected function encodeToken(array $token, $app_id = null)
    {
        $private_key = $this->publicKeyStorage->getPrivateKey($app_id);
        $algorithm   = $this->publicKeyStorage->getEncryptionAlgorithm($app_id);
     ...
     * @return array                      - The access token
     */
    protected function createPayload($app_id, $user_id, $scope = null)
    {
        // token to encrypt
        ...
        if (isset($this->config['jwt_extra_payload_callable'])) {
            ...
            $extra = call_user_func($this->config['jwt_extra_payload_callable'], $app_id, $user_id, $scope);
