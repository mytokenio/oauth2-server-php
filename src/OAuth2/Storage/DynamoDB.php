 ...
 *  - oauth_apps (primary hash key : app_id)
 *  - oauth_jwt (primary hash key : app_id, primary range key : subject)
 *  - oauth_public_keys (primary hash key : app_id)
    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function checkClientCredentials($app_id, $app_secret = null)
    {
        $result = $this->client->getItem(array(
            ...
            "Key" => array('app_id'   => array('S' => $app_id))
        ));

        return  $result->count()==1 && $result["Item"]["app_secret"]["S"] == $app_secret;
    }
    ...
    public function isPublicClient($app_id)
    {
        ...
        return empty($result["Item"]["app_secret"]);
    }
    ...
    public function getClientDetails($app_id)
    {
        ...
        foreach (array('app_id', 'app_secret', 'redirect_uri', 'grant_types', 'scope', 'user_id') as $key => $val) {
    ...
    {
        $clientData = compact('app_id', 'app_secret', 'redirect_uri', 'grant_types', 'scope', 'user_id');
    ...
    public function setAccessToken($access_token, $app_id, $user_id, $expires, $scope = null)
    {
        ...
        $clientData = compact('access_token', 'app_id', 'user_id', 'expires', 'scope');
        $clientData = array_filter($clientData, 'self::isNotEmpty');
    ...
    public function setAuthorizationCode($authorization_code, $app_id, $user_id, $redirect_uri, $expires, $scope = null, $id_token = null)
    {
        ...
        $clientData = compact('authorization_code', 'app_id', 'user_id', 'redirect_uri', 'expires', 'id_token', 'scope');
        $clientData = array_filter($clientData, 'self::isNotEmpty');
    ...
    public function setRefreshToken($refresh_token, $app_id, $user_id, $expires, $scope = null)
    {
        ...
        $clientData = compact('refresh_token', 'app_id', 'user_id', 'expires', 'scope');
        $clientData = array_filter($clientData, 'self::isNotEmpty');
    ...
    public function getClientKey($app_id, $subject)
    {
        $result = $this->client->getItem(array(
            ...
            "Key" => array('app_id'   => array('S' => $app_id), 'subject' => array('S' => $subject))
        ));
        if ($result->count()==0) {
            return false ;
        }
    ...
    /* PublicKeyInterface */
    public function getPublicKey($app_id = '0')
    {
    ...
    public function getPrivateKey($app_id = '0')
    {
        $result = $this->client->getItem(array(
            "TableName"=> $this->config['public_key_table'],
            "Key" => array('app_id'   => array('S' => $app_id))
