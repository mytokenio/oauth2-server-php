    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    {
        $this->authorizationCodes[$code] = compact('code', 'app_id', 'user_id', 'redirect_uri', 'expires', 'scope', 'id_token');
    ...
    {
        return isset($this->clientCredentials[$app_id]['app_secret']) && $this->clientCredentials[$app_id]['app_secret'] === $app_secret;
    }

    public function isPublicClient($app_id)
    {
        if (!isset($this->clientCredentials[$app_id])) {
            return false;
        }

        return empty($this->clientCredentials[$app_id]['app_secret']);
    }
    ...
    {
        if (isset($this->clientCredentials[$app_id]['grant_types'])) {
            $grant_types = explode(' ', $this->clientCredentials[$app_id]['grant_types']);
    ...
    {
        $this->clientCredentials[$app_id] = array(
            'app_id'     => $app_id,
    ...
    {
        $this->refreshTokens[$refresh_token] = compact('refresh_token', 'app_id', 'user_id', 'expires', 'scope');
    ...
    public function setAccessToken($access_token, $app_id, $user_id, $expires, $scope = null, $id_token = null)
    {
        $this->accessTokens[$access_token] = compact('access_token', 'app_id', 'user_id', 'expires', 'scope', 'id_token');
    ...
    public function getJti($app_id, $subject, $audience, $expires, $jti)
    {
        foreach ($this->jti as $storedJti) {
            if ($storedJti['issuer'] == $app_id && $storedJti['subject'] == $subject && $storedJti['audience'] == $audience && $storedJti['expires'] == $expires && $storedJti['jti'] == $jti) {
                return array(
    ...
    {
        $this->jti[] = array('issuer' => $app_id, 'subject' => $subject, 'audience' => $audience, 'expires' => $expires, 'jti' => $jti);
    }

    /*PublicKeyInterface */
    public function getPublicKey($app_id = null)
    {
        if (isset($this->keys[$app_id])) {
            return $this->keys[$app_id]['public_key'];
        }
    ...
    {
        if (isset($this->keys[$app_id])) {
            return $this->keys[$app_id]['private_key'];
        }
    ...
    {
        if (isset($this->keys[$app_id]['encryption_algorithm'])) {
            return $this->keys[$app_id]['encryption_algorithm'];
        }
