...
class JwtAccessToken implements JwtAccessTokenInterface
{
    ...
    public function getAccessToken($oauth_token)
    {
        ...
        $app_id  = isset($tokenData['aud']) ? $tokenData['aud'] : null;
        $public_key = $this->publicKeyStorage->getPublicKey($app_id);
        $algorithm  = $this->publicKeyStorage->getEncryptionAlgorithm($app_id);

        // now that we have the app_id, verify the token
    ...
    public function setAccessToken($oauth_token, $app_id, $user_id, $expires, $scope = null)
    {
        if ($this->tokenStorage) {
            return $this->tokenStorage->setAccessToken($oauth_token, $app_id, $user_id, $expires, $scope);
        }
    }
