...
class AuthorizeController implements AuthorizeControllerInterface
{
    ...
    public function handleAuthorizeRequest(RequestInterface $request, ResponseInterface $response, $is_authorized, $user_id = null)
    {
        ...
        if (empty($this->redirect_uri)) {
            $clientData              = $this->clientStorage->getClientDetails($this->app_id);
...
//        $response->setRedirect($this->config['redirect_status_code'], $uri);
    ...
    public function validateAuthorizeRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (!$app_id = $request->query('app_id', $request->request('app_id'))) {
        ...
        // Get client details
        if (!$clientData = $this->clientStorage->getClientDetails($app_id)) {
        ...
        if ($response_type == self::RESPONSE_TYPE_AUTHORIZATION_CODE) {
            ...
            if (!$this->clientStorage->checkRestrictedGrantType($app_id, 'authorization_code')) {
                $response->setRedirect($this->config['redirect_status_code'], $redirect_uri, $state, 'unauthorized_client', 'The grant type is unauthorized for this app_id', null);
            ...
            if (!$this->clientStorage->checkRestrictedGrantType($app_id, 'implicit')) {
                $response->setRedirect($this->config['redirect_status_code'], $redirect_uri, $state, 'unauthorized_client', 'The grant type is unauthorized for this app_id', null);
        ...
        if ($requestedScope) {
            ...
            $clientScope = $this->clientStorage->getClientScope($app_id);
            ...
            $defaultScope = $this->scopeUtil->getDefaultScope($app_id);
