...
//        }
        ...
        if (!$grantType instanceof ClientAssertionTypeInterface) {
            ...
            $AppId = $this->clientAssertionType->getAppId();
        }
        ...
        if ($grantType instanceof ClientAssertionTypeInterface) {
            $AppId = $grantType->getAppId();
        } else {
            // validate the Client ID (if applicable)
            if (!is_null($storedAppId = $grantType->getAppId()) && $storedAppId != $AppId) {
         ...
         */
        if (!$this->clientStorage->checkRestrictedGrantType($AppId, $grantTypeIdentifier)) {
            $response->setError(400, 'unauthorized_client', 'The grant type is unauthorized for this app_id');
        ...
        if ($requestedScope) {
            ...
            if ($availableScope) {
                ...
                if ($clientScope = $this->clientStorage->getClientScope($AppId)) {
            ...
            $defaultScope = $this->scopeUtil->getDefaultScope($AppId);
        ...
        return $grantType->createAccessToken($this->accessToken, $AppId, $grantType->getUserId(), $requestedScope);
    }
