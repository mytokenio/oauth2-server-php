...
class ClientCredentials extends HttpBasic implements GrantTypeInterface
{
     ...
     * @return array
     */
    public function createAccessToken(AccessTokenInterface $accessToken, $app_id, $user_id, $scope)
    {
        /**
        ...
        return $accessToken->createAccessToken($app_id, $user_id, $scope, $includeRefreshToken);
    }

    private function loadClientData()
    {
        if (!$this->clientData) {
            $this->clientData = $this->storage->getClientDetails($this->getAppId());
        }
    }
}
