...
class HttpBasic implements ClientAssertionTypeInterface
{
    ...
    public function validateRequest(RequestInterface $request, ResponseInterface $response)
    {
        ...
        if (!isset($clientData['app_id'])) {
            throw new LogicException('the clientData array must have "app_id" set');
        }

        if (!isset($clientData['app_secret']) || $clientData['app_secret'] == '') {
            ...
            if (!$this->storage->isPublicClient($clientData['app_id'])) {
                $response->setError(400, 'invalid_client', 'This client is invalid or must authenticate using a client secret');

                return false;
            }
        } elseif ($this->storage->checkClientCredentials($clientData['app_id'], $clientData['app_secret']) === false) {
     ...
     * auth or POST data.
     *
     * According to the spec (draft 20), the app_id can be provided in
     ...
     * @code
     *     return array(
     *         "app_id"     => app_id,        // REQUIRED the client id
     *         "app_secret" => app_secret,    // OPTIONAL the client secret (may be omitted for public clients)
     *     );
     * @endcode
     *
    ...
    public function getClientCredentials(RequestInterface $request, ResponseInterface $response = null)
    {
        if (!is_null($request->headers('PHP_AUTH_USER')) && !is_null($request->headers('PHP_AUTH_PW'))) {
            return array('app_id' => $request->headers('PHP_AUTH_USER'), 'app_secret' => $request->headers('PHP_AUTH_PW'));
        }

        if ($this->config['allow_credentials_in_request_body']) {
            // Using POST for HttpBasic authorization is not recommended, but is supported by specification
            if (!is_null($request->request('app_id'))) {
                /**
                 * app_secret can be null if the client's password is an empty string
                 * @see http://tools.ietf.org/html/rfc6749#section-2.3.1
                 */
                return array('app_id' => $request->request('app_id'), 'app_secret' => $request->request('app_secret'));
            }
        }
