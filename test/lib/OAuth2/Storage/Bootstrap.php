...
class Bootstrap
{
    ...
    private function createCassandraDb(Cassandra $storage)
    {
        ...
        $cf->insert("oauth_public_keys:AppId_One", array('__data' => json_encode(array("public_key" => "client_1_public", "private_key" => "client_1_private", "encryption_algorithm" => "RS256"))));
        $cf->insert("oauth_public_keys:AppId_Two", array('__data' => json_encode(array("public_key" => "client_2_public", "private_key" => "client_2_private", "encryption_algorithm" => "RS256"))));
    ...
    public function runPdoSql(\PDO $pdo)
    {
        ...
        // set up clients
        $sql = 'INSERT INTO oauth_apps (app_id, app_secret, scope, grant_types) VALUES (?, ?, ?, ?)';
        ...
        // set up misc
        $sql = 'INSERT INTO oauth_access_tokens (access_token, app_id, expires, user_id) VALUES (?, ?, ?, ?)';
        ...
        $sql = 'INSERT INTO oauth_authorization_codes (authorization_code, app_id, expires) VALUES (?, ?, ?)';
        ...
        $sql = 'INSERT INTO oauth_public_keys (app_id, public_key, private_key, encryption_algorithm) VALUES (?, ?, ?, ?)';
        $pdo->prepare($sql)->execute(array('AppId_One', 'client_1_public', 'client_1_private', 'RS256'));
        $pdo->prepare($sql)->execute(array('AppId_Two', 'client_2_public', 'client_2_private', 'RS256'));

        $sql = 'INSERT INTO oauth_public_keys (app_id, public_key, private_key, encryption_algorithm) VALUES (?, ?, ?, ?)';
        $pdo->prepare($sql)->execute(array(null, $this->getTestPublicKey(), $this->getTestPrivateKey(), 'RS256'));

        $sql = 'INSERT INTO oauth_jwt (app_id, subject, public_key) VALUES (?, ?, ?)';
    ...
    {
        $db->set('oauth_apps-oauth_test_client',json_encode(array(
            'app_id' => "oauth_test_client",
            'app_secret' => "testpass",
        ...
        if (!$this->dynamodb) {
            ...
            if (class_exists('\Aws\DynamoDb\DynamoDbClient')) {
                if ($client = $this->getDynamoDbClient()) {
                    ...
                    $config = array(
                        'client_table' => $prefix.'oauth_apps',
    ...
    {
        $tablesList = explode(' ', 'oauth_access_tokens oauth_authorization_codes oauth_apps oauth_jwt oauth_public_keys oauth_refresh_tokens oauth_scopes oauth_users');
    ...
    private function createDynamoDb(\Aws\DynamoDb\DynamoDbClient $client, $prefix = null)
    {
        ...
        $client->createTable(array(
            'TableName' => $prefix.'oauth_apps',
            'AttributeDefinitions' => array(
                array('AttributeName' => 'app_id','AttributeType' => 'S')
            ),
            'KeySchema' => array(array('AttributeName' => 'app_id','KeyType' => 'HASH')),
            'ProvisionedThroughput' => array('ReadCapacityUnits'  => 1,'WriteCapacityUnits' => 1)
        ));
        ...
        $client->createTable(array(
            ...
            'KeySchema' => array(
                array('AttributeName' => 'app_id','KeyType' => 'HASH'),
    ...
    private function populateDynamoDb($client, $prefix = null)
    {
        ...
        $client->putItem(array(
            'TableName' => $prefix.'oauth_apps',
            'Item' => array(
                'app_id' => array('S' => 'Test Client ID'),
                'app_secret' => array('S' => 'TestSecret'),
        ...
        $client->putItem(array(
            'TableName' => $prefix.'oauth_apps',
            'Item' => array(
                'app_id' => array('S' => 'Test Client ID 2'),
                'app_secret' => array('S' => 'TestSecret'),
        ...
        $client->putItem(array(
            'TableName' => $prefix.'oauth_apps',
            'Item' => array(
                'app_id' => array('S' => 'Test Default Scope Client ID'),
                'app_secret' => array('S' => 'TestSecret'),
                'scope' => array('S' => 'clientscope1 clientscope2')
            )
        ));
        ...
        $client->putItem(array(
            'TableName' => $prefix.'oauth_apps',
            'Item' => array(
                'app_id' => array('S' => 'oauth_test_client'),
                'app_secret' => array('S' => 'testpass'),
